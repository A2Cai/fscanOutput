...
#输出识别到的系统
def Oslist(datalist):
    replaceList = ["[*]", '\t', "\x01", '\x02', '(', ')', 'OsInfo ']
    sheetList = [['ip', 'os']]
    for t in datalist:
        p = re.findall(r"\[\*]\sOsInfo\s\d+\.\d+\.\d+\.\d+.*", t)
        if len(p) != 0:
            p1 = list(p)
            for u in p1:
                ip = re.findall(r"\d+\.\d+\.\d+\.\d+", u)
...
#输出poc漏洞列表
def Bug_PocList(datastr):
    sheetList = [['url', 'bug_poc']]  # 表头
    p = re.findall(r"^\[\+\]\s\[发现漏洞\]\s目标:\s(.*)\n\s+漏洞类型:\s(.*)\n\s+漏洞名称:\s(.*)", datastr, re.MULTILINE)
    bugs = []
    for i in p:
        bugs.append([i[0].strip(), i[1].strip()])
    for bug in bugs:
        sheetList.append(bug)
...
def GetTitle(datalist):
    ...
    for i in datalist:
        p = re.findall(r'\[\*]\s网站标题.*', i)
        if len(p) != 0:
            p1 = list(p)
            for u in p1:
                all_url = re.findall(r"http[^\s]+", u)
                url = [all_url[0]]
                code = re.findall(r'(?<=状态码:)[^\s]+', u)
                len1 = re.findall(r'(?<=长度:)[^\s]+', u)
                title = re.findall(r'(?<=标题:).*', u)
                url.append(code[0])
...
def FingerOut(datastr):
    ...
    for i in datastr:
        p = re.findall(r'.*发现指纹.*', i)
        if len(p) != 0:
            p1 = list(p)
            for u in p1:
                url = re.findall(r'http[^\s]+', u)
                finger = re.findall(r'指纹: \[(.*?)\]', u.split(url[0])[-1].strip())[0]
                url.append(finger)
                sheetList.append(url)
...
if __name__ == "__main__":
    ...
    wb.remove(ws5)
    input_filename = sys.argv[1].split(".txt")[0]
    Output_xlsx = (f"%s_{time.strftime('%Y-%m-%d', time.localtime())}.xlsx" % input_filename)
    wb.save(Output_xlsx)
    print("[+]文件处理结果如下······\n")
